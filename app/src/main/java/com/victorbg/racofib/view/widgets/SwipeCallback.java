...
public class SwipeCallback extends ItemTouchHelper.SimpleCallback {
  ...
  public SwipeCallback(
      ItemSwipeCallback itemSwipeCallback, ItemSwipeDrawableCallback itemSwipeDrawableCallback) {
  ...
  public void onChildDraw(
    ...
    if (Math.abs(dX) > Math.abs(dY)) {
      ...
      // Prevent flashing the next state when the user has no control on the animation as it is
      // returning to its original state
      if (isCurrentlyActive) {
        bgColorLeft =
            bgColorRight = itemSwipeDrawableCallback.getColor(viewHolder.getAdapterPosition());
        leaveBehindDrawableLeft =
            leaveBehindDrawableRight =
                itemSwipeDrawableCallback.getDrawable(viewHolder.getAdapterPosition());
      }
