 ...
 * (if provided)
 *
 * <p>The state is represented in {@link Rule}, that every destination must have in order to apply
 * them when {@link #navigate(int, Bundle, boolean)} is called with that destination. It is possible
 * that there are destinations with no {@link Rule} attached, in that case it is ignores but is
 * hardly recommended to attach one to them
 */
public class BottomBarNavigator {
  ...
  public static BottomBarNavigator createNavigator(
      Activity context, FragNav fragmentNavigator, BottomAppBar bottomAppBar) {
    return new BottomBarNavigator(context, fragmentNavigator, bottomAppBar, null);
  }

  public static BottomBarNavigator createNavigatorWithFAB(
      Activity context,
      FragNav fragmentNavigator,
      BottomAppBar bottomAppBar,
      FloatingActionButton floatingActionButton) {
    return new BottomBarNavigator(context, fragmentNavigator, bottomAppBar, floatingActionButton);
  }

  private BottomBarNavigator(
      Activity context,
      FragNav fragmentNavigator,
      BottomAppBar bottomAppBar,
      FloatingActionButton floatingActionButton) {
  ...
  private void applyNewNavigationMode(int currentNavigationMode, int nextNavigationMode) {
    if (currentNavigationMode == NAVIGATION_MODE_BACK
        && nextNavigationMode == NAVIGATION_MODE_NAVIGATION) {
    ...
    } else if (nextNavigationMode == NAVIGATION_MODE_BACK
        && currentNavigationMode == NAVIGATION_MODE_NAVIGATION) {
      if (bottomAppBar.getNavigationIcon() == null) {
        bottomAppBar.setNavigationIcon(navigationIcon);
        setNavIconProgress(DRAWER_MODE_NAVIGATION_PROGRESS, false);
      } else {
      ...
      bottomAppBar.setNavigationIcon(
          nextNavigationMode == NAVIGATION_MODE_NONE ? null : navigationIcon);
    }
  }
  ...
  private void setNavIconProgress(
      @FloatRange(from = 0.0, to = 1.0) float progress, boolean animate) {
    ...
    if (navigationIcon.getProgress() != progress) {
      if (!animate) {
        ...
        ValueAnimator valueAnimator =
            ValueAnimator.ofFloat(navigationIcon.getProgress(), progress).setDuration(250);
        valueAnimator.addUpdateListener(
            animation -> navigationIcon.setProgress((Float) animation.getAnimatedValue()));
  ...
  public static class Rule {
    ...
    public Rule(
        int fabAlignmentMode, int navigationMode, int menu, int fabImage, int fabVisibility) {
