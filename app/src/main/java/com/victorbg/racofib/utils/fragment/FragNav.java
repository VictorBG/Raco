...
public class FragNav {
  ...
  private void internalAddFragment(int id, Fragment fragment) {
    if (fragmentArray.indexOfKey(id) < 0) {
      ...
      if (overrideFragment) {
        ...
        Timber.w(
            "A fragment with that ID already exists. If you want to override it, call setOverride(true)");
      }
    }
  }
  ...
  public void replaceFragment(int id, @Nullable Bundle arguments) {
    ...
    if (fr != null) {
      ...
      fragmentManager
          .beginTransaction()
          .add(R.id.contentContainer, arrayFragment, tag)
          .setTransition(FragmentTransaction.TRANSIT_FRAGMENT_FADE)
          .commit();
    }
    ...
    if (lastTagSelected != null) {
      if (popNextTransaction.getValue()) {
        fragmentManager
            .beginTransaction()
            .detach(fragmentManager.findFragmentByTag(lastTagSelected))
            .commit();
      } else {
        fragmentManager
            .beginTransaction()
            .hide(fragmentManager.findFragmentByTag(lastTagSelected))
            .commit();
      }
    }
