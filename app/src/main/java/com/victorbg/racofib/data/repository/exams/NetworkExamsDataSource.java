...
public class NetworkExamsDataSource implements DataSource<Resource<List<Exam>>> {
  ...
  public NetworkExamsDataSource(
      ApiService apiService,
      ExamDao examDao,
      SaveOfflineData<List<Exam>> saveData,
   ...
   *
   * <p>It first gets the last semester, in order to get the last exams, and then the exams of this
   * semester with the subjects short name as filter.
   *
   * <p>Once retrieved it is stored in the database (if {@link SaveOfflineData} provided) and then
   * returns the LiveData with the current fetched values.
   *
   * @return
