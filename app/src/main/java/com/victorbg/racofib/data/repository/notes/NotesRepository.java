...
public class NotesRepository extends Repository {
  ...
  public NotesRepository(
      AppDatabase appDatabase, AppExecutors appExecutors, ApiService apiService, Context context) {
    this.appDatabase = appDatabase;
    this.apiService = apiService;
    this.appExecutors = appExecutors;
    this.context = context;

    this.networkNotesDataSource =
        new NetworkNotesDataSource(
            apiService,
            appDatabase.notesDao(),
            new NotesSaveOfflineData(appDatabase, appExecutors));
  }

  /**
   * Retrieves the notes from 3 different sources: cache, database and remote based on the current
   * state of the data, If it has never been retrieved it is fetched from remote (which in case of
   * fail will return from database), if it should not be fetch from remote (less than 15 minutes
   * between calls, {@link #networkRateLimiter}) it is fetched from database, and in case it is
   * being fetched a lot from the database (calls within a minute, {@link #databaseRateLimiter}) it
   * is retrieved from the local cache (which will never be dirty).
   *
   * @return
   */
  @MainThread
