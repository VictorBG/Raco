...
public class NotesSaveOfflineData implements SaveOfflineData<List<Note>> {
  ...
  public void saveData(List<Note> data) {
    appExecutors.executeOnDisk(() -> appDatabase.runInTransaction(() -> internalSave(data)));
  }

  /**
   * Saves the returned data from the API into the database setting the correct color to every item.
   *
   * <p>If there was an error assigning the colors just insert the data with the default color value
   *
   * @param data
   */
  @WorkerThread
  private void internalSave(List<Note> data) {
    ...
    compositeDisposable.add(
        subjectsDao
            ...
            .subscribe(notesDao::insertNotes, error -> notesDao.insertNotes(data)));
  }
}
