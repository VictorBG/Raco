  ...
  public LiveData<Resource<String>> authUser(Context context, String code) {
    ...
    appExecutors.networkIO().execute(() ->
        compositeDisposable.add(apiService.getAccessToken(
            ...
            token -> apiService.getUser(NetworkUtils.prepareToken(token.getAccessToken()), "json")
                .flatMap(user -> {
                  appExecutors.diskIO().execute(() -> userDao.insert(user));
                  appExecutors.mainThread().execute(() -> result.setValue(
                      Resource.loading(context.getString(R.string.fetching_subjects_message))));

                  return apiService
                      .getSubjects(NetworkUtils.prepareToken(token.getAccessToken()), "json")
                      .flatMap(subjects -> {
                        ...
                        appExecutors.mainThread().execute(() -> result.setValue(
                            Resource
                                .loading(context.getString(R.string.fetching_schedule_message))));

                        return apiService
                            .getSubjectsSchedule(NetworkUtils.prepareToken(token.getAccessToken()),
                                "json")
                            .flatMap(timetable -> {
                              if (timetable != null) {
                                appExecutors.diskIO()
                                    .execute(() -> subjectScheduleDao.insert(timetable.result));
                              }
            ...
            .subscribe(tokenResponse -> {
              ...
              appExecutors.mainThread().execute(() -> result
                  .setValue(Resource.error("An error has occurred: " + error.getMessage(), null)));
            })));
