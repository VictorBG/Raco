...
/**
 * TODO: This repository is a mess and bad structured, it even has functionalities that should not
 * be here...
 */
@Singleton
public class UserRepository {
        ...
        loadUser(getToken())
            ...
            .subscribe(
                tokenResponse -> {
                  ...
                  getUser();
                  appExecutors.executeOnMainThread(
                      () -> loadingStatus.setValue(Resource.success(null)));
                },
                error ->
                    appExecutors.executeOnMainThread(
                        () ->
   ...
   *
   * <p>Once the user has been authorized successfully it downloads all the necessary data for the
   * app to run like {@link User}, the {@link com.victorbg.racofib.data.model.subject.Subject}s and
   * the {@link SubjectSchedule} which indicates the schedule of the user.
   *
                ...
                tokenResponse -> {
                  ...
                  WorkManager.getInstance()
                      .enqueueUniquePeriodicWork(
                          ...
                          new PeriodicWorkRequest.Builder(SeasonCheckTask.class, 7, TimeUnit.DAYS)
                              .build());
                },
                error ->
                    appExecutors.executeOnMainThread(
  ...
  private Single<TokenResponse> loadSubjectsSchedule(TokenResponse token) {
    ...
    return apiService
        .getSubjectsSchedule(getToken(token))
        .map(t -> t.result)
        .doOnSuccess(
            timetable -> {
                ...
                .addOnCompleteListener(
                    task ->
                        Optional.of(task)
                            .ifPresent(
                                (t) -> {
                                  Optional.ofNullable(t.getResult().getData())
                                      .ifPresent(
                                          map ->
                                              s.grades =
                                                  map.keySet().stream()
  ...
  /**
   * Executes a special command for the UserRepository. This command is composed by the general
   * command (the API call) and by a success callback that is executed once the command has
   * finished.
   *
   ...
   */
  private <A> Single<A> executeCommandNoMap(
      @NonNull Function<Single<A>, ?> command, @NonNull Function<Single<A>, A> doOnSuccess) {
  ...
  /**
   * Executes a special command for the UserRepository. This command is composed by the general
   * command (the API call), a success callback that is executed once the command has finished and a
   * flatMap that is executed before returning the value
   *
   * @param command Command to execute
   * @param doOnSuccess Command to execute when on success
   ...
   * @return A single containing the value of the call
   */
  private <A, B> Single<B> executeCommandMap(
      @NonNull Function<Single<A>, ?> command,
      @NonNull Function<Single<A>, A> doOnSuccess,
      @NonNull Function<Single<B>, A> mapFunction) {
    return executeCommandNoMap(command, doOnSuccess).flatMap(mapFunction::run);
  }

  /**
   * Saves on disk the given data
   *
  ...
  private static class UIMessaging {
    ...
    public UIMessaging(
        Context context,
        AppExecutors appExecutors,
        MutableLiveData<Resource<String>> loadingStatus) {
    ...
    private void setUIMessage(@StringRes int id) {
      appExecutors.executeOnMainThread(
          () -> loadingStatus.setValue(Resource.loading(context.getString(id))));
    }
