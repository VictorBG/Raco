...
public class AttachmentDownload {
    ...
    public void download(Attachment attachment, Activity activity) {
        ...
        DownloadFile downloadFile = new DownloadFile(context, attachment, () -> {
    ...
    /**
     * Starts downloading the next file in the queue if there is no current download.
     * If there is a current download skips starting
     */
    ...
    private final class DownloadFile {
        ...
        protected DownloadFile(Context context, Attachment attachment, AttachmentDownload.DownloadListener listener) {
            this.context = context;
        ...
        private void openFile(String f, String fileMimeType) {
            if (f != null) {
                try {
                    Uri uri = FileProvider.getUriForFile(context, BuildConfig.APPLICATION_ID + ".fileprovider", new File(f));
