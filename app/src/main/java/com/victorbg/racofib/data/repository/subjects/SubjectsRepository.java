...
public class SubjectsRepository {
    ...
    /**
     * Returns a Resource with the state of the desired subject, which is retrieved from the API every time it is called and it is not saved in the
     * local database, is not considered a critial information to know even when internet is not available.
     *
    ...
    public LiveData<Resource<Subject>> getSubject(String subject) {
        ...
        appExecutors.executeOnNetwork(() ->
                compositeDisposable.add(apiService.getSubject(subject)
